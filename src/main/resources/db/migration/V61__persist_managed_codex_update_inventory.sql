...
CREATE TABLE worker_codex_update_plan (
    ...
    CONSTRAINT ck_worker_codex_update_plan_state
        CHECK (state IN ('READY', 'BLOCKED', 'ACTIVATED', 'ROLLED_BACK')),
    ...
    CONSTRAINT ck_worker_codex_update_plan_projection
        CHECK (
            (state IN ('READY', 'ACTIVATED', 'ROLLED_BACK') AND compatibility_state = 'COMPATIBLE'
...
CREATE TABLE worker_codex_rollback_authorization (
    authorization_id UUID PRIMARY KEY,
    ...
    idempotency_key UUID NOT NULL,
    current_inventory_id UUID NOT NULL,
    previous_inventory_id UUID NOT NULL,
    authorization_digest_sha256 VARCHAR(64) NOT NULL,
    expires_at TIMESTAMPTZ NOT NULL,
    consumed_at TIMESTAMPTZ,
    consumed_rollback_id UUID,
    created_at TIMESTAMPTZ NOT NULL,
    CONSTRAINT fk_worker_codex_rollback_auth_activation
        FOREIGN KEY (activation_id) REFERENCES worker_codex_activation_operation (activation_id)
        ON DELETE RESTRICT,
    CONSTRAINT fk_worker_codex_rollback_auth_current
        FOREIGN KEY (worker_id, current_inventory_id)
        REFERENCES worker_codex_release_inventory (worker_id, inventory_id)
        ON DELETE RESTRICT,
    CONSTRAINT fk_worker_codex_rollback_auth_previous
        FOREIGN KEY (worker_id, previous_inventory_id)
        REFERENCES worker_codex_release_inventory (worker_id, inventory_id)
        ON DELETE RESTRICT,
    CONSTRAINT fk_worker_codex_rollback_auth_operator
        FOREIGN KEY (requested_by) REFERENCES operator_account (id) ON DELETE RESTRICT,
    CONSTRAINT uk_worker_codex_rollback_auth_idempotency
        UNIQUE (requested_by, idempotency_key),
    CONSTRAINT uk_worker_codex_rollback_auth_exact
        UNIQUE (authorization_id, activation_id, plan_id, worker_id),
    CONSTRAINT ck_worker_codex_rollback_auth_distinct
        CHECK (current_inventory_id <> previous_inventory_id),
    CONSTRAINT ck_worker_codex_rollback_auth_digest
        CHECK (authorization_digest_sha256 ~ '^[0-9a-f]{64}$'),
    CONSTRAINT ck_worker_codex_rollback_auth_expiry CHECK (expires_at > created_at),
    CONSTRAINT ck_worker_codex_rollback_auth_consumption
        CHECK ((consumed_at IS NULL AND consumed_rollback_id IS NULL)
            OR (consumed_at IS NOT NULL AND consumed_rollback_id IS NOT NULL
                AND consumed_at >= created_at))
);

CREATE TABLE worker_codex_rollback_operation (
    ...
    idempotency_key UUID NOT NULL,
    state VARCHAR(16) NOT NULL,
    link_restore VARCHAR(16) NOT NULL,
    worker_service_restart VARCHAR(16) NOT NULL,
    affected_services VARCHAR(120) NOT NULL,
    app_server_services_restarted INTEGER NOT NULL,
    current_before_fingerprint VARCHAR(64) NOT NULL,
    previous_before_fingerprint VARCHAR(64) NOT NULL,
    current_after_fingerprint VARCHAR(64) NOT NULL,
    previous_after_fingerprint VARCHAR(64) NOT NULL,
    ...
    created_at TIMESTAMPTZ NOT NULL,
    completed_at TIMESTAMPTZ NOT NULL,
    CONSTRAINT fk_worker_codex_rollback_auth
        FOREIGN KEY (authorization_id, activation_id, plan_id, worker_id)
        REFERENCES worker_codex_rollback_authorization
            (authorization_id, activation_id, plan_id, worker_id)
        ON DELETE RESTRICT,
    CONSTRAINT fk_worker_codex_rollback_operator
        FOREIGN KEY (requested_by) REFERENCES operator_account (id) ON DELETE RESTRICT,
    CONSTRAINT uk_worker_codex_rollback_idempotency UNIQUE (requested_by, idempotency_key),
    CONSTRAINT ck_worker_codex_rollback_state CHECK (state = 'ROLLED_BACK'),
    CONSTRAINT ck_worker_codex_rollback_effect
        CHECK (link_restore = 'PASS' AND worker_service_restart = 'PASS'
            AND affected_services = 'atenea-agent-run-worker-v1.service'
            AND app_server_services_restarted = 0),
    CONSTRAINT ck_worker_codex_rollback_fingerprints
        CHECK (current_before_fingerprint ~ '^[0-9a-f]{64}$'
            AND previous_before_fingerprint ~ '^[0-9a-f]{64}$'
            AND current_after_fingerprint ~ '^[0-9a-f]{64}$'
            AND previous_after_fingerprint ~ '^[0-9a-f]{64}$'),
    CONSTRAINT ck_worker_codex_rollback_no_values CHECK (values_exposed = FALSE),
    CONSTRAINT ck_worker_codex_rollback_time CHECK (completed_at >= created_at)
);

CREATE INDEX idx_worker_codex_rollback_worker_created
    ON worker_codex_rollback_operation (worker_id, created_at DESC);

...
- [x] 2.1 Capture timestamped pre-change evidence and rollback copies of sshd, UFW and managed systemd configuration.
- [x] 2.2 Set the canonical hostname and verify local resolution without changing public DNS.
- [x] 2.3 Create the named `jose` sudo administrator, install the approved ED25519 key and prove a fresh independent login plus non-interactive sudo.
- [x] 2.4 Create the `atenea` group, no-login `atenea-worker` system account and least-privilege `/srv/atenea` and `/etc/atenea-worker` skeleton.
- [x] 2.5 Apply supported security updates and install baseline administration, RAID/NVMe monitoring and Tailscale-package prerequisites with automatic reboot disabled.

## 3. SSH and firewall anti-lockout transition

- [x] 3.1 Render the sshd drop-in, validate with `sshd -t` and record the effective policy before reload.
- [x] 3.2 Reload sshd while the recovery connection remains open and prove new named-admin and root key-only connections.
- [x] 3.3 Stage a deny-by-default IPv4/IPv6 UFW policy with rate-limited public SSH and no worker/runtime ports.
- [x] 3.4 Enable UFW and prove a new SSH connection before closing either recovery session.
- [x] 3.5 Verify password, keyboard-interactive and X11 access are disabled and the public listener set matches the phase contract.
...
- [x] 5.1 Install and enable the worker health script, systemd service and timer.
- [x] 5.2 Verify healthy RAID `[UU]`, no resync, NVMe SMART, disk thresholds, time sync, sshd syntax, firewall and required identities in human and JSON output.
- [x] 5.3 Trigger and record safe negative-path checks without degrading the live RAID or access path.
- [x] 5.4 Perform a controlled reboot and re-verify host key, named admin, root break-glass, private connectivity when enrolled, firewall, RAID/SMART and timer state.
- [x] 5.5 Record resulting configuration evidence and any deferred gates in the programme ledger.
